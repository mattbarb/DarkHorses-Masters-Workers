#!/usr/bin/env python3
"""
Calculate Owner Statistics from Database
=========================================
...
------
    ...
    python3 scripts/statistics_workers/calculate_owner_statistics.py --limit 100

    # Resume from checkpoint
    python3 scripts/statistics_workers/calculate_owner_statistics.py --resume
...
logger = get_logger('calculate_owner_statistics')

CHECKPOINT_FILE = 'logs/owner_statistics_checkpoint.json'
BATCH_SIZE = 100


class OwnerStatisticsCalculator:
    """Calculate owner statistics from database"""
    ...
    def calculate_owner_statistics(self, owner_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Error calculating statistics for owner {owner_id}: {e}")
            return None

    def update_owner_stats(self, stats: Dict) -> bool:
        """Update statistics in ra_mst_owners table"""
        try:
            owner_id = stats.pop('id')
            result = self.db_client.client.table('ra_mst_owners')\
                .update(stats)\
                .eq('id', owner_id)\
                .execute()
            return True
        except Exception as e:
            logger.error(f"Error updating stats for owner {stats.get('id', 'unknown')}: {e}")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Calculate and populate owner statistics')
    parser.add_argument('--limit', type=int, help='Limit number of owners to process (for testing)')
    parser.add_argument('--resume', action='store_true', help='Resume from last checkpoint')
    args = parser.parse_args()

    logger.info("=" * 80)
    logger.info("OWNER STATISTICS CALCULATOR")
    logger.info("=" * 80)
    ...
    calculator = OwnerStatisticsCalculator(db_client)
    ...
    try:
        query = db_client.client.table('ra_mst_owners').select('id')
    ...
    for i in range(start_index, len(owners)):
        owner = owners[i]
        owner_id = owner['id']

        logger.info(f"[{i+1}/{len(owners)}] Processing owner {owner_id}...")

        # Calculate statistics
        stats = calculator.calculate_owner_statistics(owner_id)

        if stats:
            # Update database
            if calculator.update_owner_stats(stats):
    ...
    logger.info("\n" + "=" * 80)
    logger.info("OWNER STATISTICS CALCULATOR COMPLETE")
    logger.info("=" * 80)
