"""
Trainers Statistics Worker

Populates statistics fields in ra_trainers table:
...
def calculate_trainer_statistics(trainer_id: str, trainer_name: str, api_client: RacingAPIClient) -> Optional[Dict]:
    """
    Calculate statistics for a single trainer

    Args:
        trainer_id: Trainer ID
        trainer_name: Trainer name (for logging)
    ...
    try:
        ...
        logger.debug(f"Fetching results for {trainer_name} ({trainer_id}) from {start_date} to {end_date}")
        ...
        while True:
            results = api_client.get_trainer_results(
        ...
        logger.debug(f"Fetched {len(all_results)} results for {trainer_name}")
        ...
        for race in all_results:
            ...
            trainer_runners = [r for r in runners if r.get('trainer_id') == trainer_id]
        ...
        logger.error(f"Error calculating statistics for {trainer_name} ({trainer_id}): {e}")
        return None


def process_trainers_batch(trainers: List[Dict], api_client: RacingAPIClient, db_client: SupabaseReferenceClient) -> Dict:
    """
    Process a batch of trainers

    Args:
        trainers: List of trainer records
        api_client: Racing API client
    ...
    for trainer in trainers:
        ...
        logger.info(f"Processing {trainer_name} ({trainer_id})...")

        # Calculate statistics
        stats = calculate_trainer_statistics(trainer_id, trainer_name, api_client)

        if stats:
            # Add trainer ID to update record
            stats['id'] = trainer_id
    ...
    # We only want to update statistics for existing trainers, not insert new ones
    if updates:
        try:
            for update in updates:
                trainer_id = update.pop('id')  # Remove id from update data
                result = db_client.client.table('ra_trainers').update(update).eq('id', trainer_id).execute()
                if result.data:
                    batch_stats['updated'] += 1
            logger.info(f"Updated {batch_stats['updated']} trainers in database")
        except Exception as e:
            logger.error(f"Error updating trainers batch: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Calculate and update trainer statistics')
    parser.add_argument('--limit', type=int, help='Limit number of trainers to process (for testing)')
    args = parser.parse_args()

    logger.info("=" * 80)
    logger.info("TRAINERS STATISTICS WORKER")
    logger.info("=" * 80)
    ...
    # Fetch trainers (excluding test data)
    logger.info("Fetching trainers from database...")
    try:
        query = db_client.client.table('ra_trainers').select('id, name').not_.like('id', '**TEST**%')
    ...
    for i in range(0, len(trainers), batch_size):
        batch = trainers[i:i + batch_size]
        batch_num = (i // batch_size) + 1
        total_batches = (len(trainers) + batch_size - 1) // batch_size

        logger.info(f"\nProcessing batch {batch_num}/{total_batches} ({len(batch)} trainers)...")

        batch_stats = process_trainers_batch(batch, api_client, db_client)
    ...
    logger.info("\n" + "=" * 80)
    logger.info("TRAINERS STATISTICS WORKER COMPLETE")
    logger.info("=" * 80)
