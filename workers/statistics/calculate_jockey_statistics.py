#!/usr/bin/env python3
"""
Calculate Jockey Statistics from Database
==========================================
...
1. LIFETIME STATISTICS:
   - total_rides, total_wins, total_places (top 3), total_seconds, total_thirds
   - win_rate, place_rate
...
------
    ...
    python3 scripts/statistics_workers/calculate_jockey_statistics.py --limit 100

    # Resume from checkpoint
    python3 scripts/statistics_workers/calculate_jockey_statistics.py --resume
...
logger = get_logger('calculate_jockey_statistics')

CHECKPOINT_FILE = 'logs/jockey_statistics_checkpoint.json'
BATCH_SIZE = 100


class JockeyStatisticsCalculator:
    """Calculate jockey statistics from database"""
    ...
    def calculate_jockey_statistics(self, jockey_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            for runner in runners.data:
                ...
                if race_date_str:
                    try:
                        ...
                        if not last_ride_date or race_date > last_ride_date:
                            last_ride_date = race_date
            ...
            # Calculate rates
            win_rate = round((total_wins / total_rides) * 100, 2) if total_rides > 0 else None
            place_rate = round((total_places / total_rides) * 100, 2) if total_rides > 0 else None
            recent_14d_win_rate = round((recent_14d_wins / recent_14d_rides) * 100, 2) if recent_14d_rides > 0 else None
            recent_30d_win_rate = round((recent_30d_wins / recent_30d_rides) * 100, 2) if recent_30d_rides > 0 else None

            # Calculate days since
            days_since_last_ride = (self.today - last_ride_date).days if last_ride_date else None
            ...
            logger.error(f"Error calculating statistics for jockey {jockey_id}: {e}")
            return None

    def update_jockey_stats(self, stats: Dict) -> bool:
        """Update statistics in ra_mst_jockeys table"""
        try:
            jockey_id = stats.pop('id')
            result = self.db_client.client.table('ra_mst_jockeys')\
                .update(stats)\
                .eq('id', jockey_id)\
                .execute()
            return True
        except Exception as e:
            logger.error(f"Error updating stats for jockey {stats.get('id', 'unknown')}: {e}")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Calculate and populate jockey statistics')
    parser.add_argument('--limit', type=int, help='Limit number of jockeys to process (for testing)')
    parser.add_argument('--resume', action='store_true', help='Resume from last checkpoint')
    args = parser.parse_args()

    logger.info("=" * 80)
    logger.info("JOCKEY STATISTICS CALCULATOR")
    logger.info("=" * 80)
    ...
    calculator = JockeyStatisticsCalculator(db_client)
    ...
    try:
        query = db_client.client.table('ra_mst_jockeys').select('id')
    ...
    # Process jockeys in batches
    start_time = datetime.utcnow()
    start_index = checkpoint['last_processed_index']

    for i in range(start_index, len(jockeys)):
        jockey = jockeys[i]
        jockey_id = jockey['id']

        logger.info(f"[{i+1}/{len(jockeys)}] Processing jockey {jockey_id}...")

        # Calculate statistics
        stats = calculator.calculate_jockey_statistics(jockey_id)

        if stats:
            # Update database
            if calculator.update_jockey_stats(stats):
    ...
    logger.info("\n" + "=" * 80)
    logger.info("JOCKEY STATISTICS CALCULATOR COMPLETE")
    logger.info("=" * 80)
