-- ============================================================================
-- SMALLEST POSSIBLE BATCH - Run this 100+ times (FIXED VERSION)
-- ============================================================================
...
-- ============================================================================
-- FIX: Added validation to skip invalid numeric values like "-"
-- ============================================================================
...
WHERE runner_id IN (
    ...
    WHERE api_data->>'weight_lbs' IS NOT NULL
      AND api_data->>'weight_lbs' ~ '^[0-9]+$'  -- Only valid integers
      AND weight IS NULL
    LIMIT 1000
);
...
-- Update 1000 starting_price_decimal records (skip "-" and invalid values)
UPDATE ra_runners
...
WHERE runner_id IN (
    ...
    WHERE api_data->>'sp_dec' IS NOT NULL
      AND api_data->>'sp_dec' != '-'
      AND api_data->>'sp_dec' ~ '^[0-9]+\.?[0-9]*$'  -- Only valid decimals
...
-- Update 1000 overall_beaten_distance records (skip "-" and invalid values)
UPDATE ra_runners
...
WHERE runner_id IN (
    ...
    WHERE api_data->>'ovr_btn' IS NOT NULL
      ...
      AND api_data->>'ovr_btn' ~ '^[0-9]+\.?[0-9]*$'  -- Only valid decimals
...
WHERE runner_id IN (
    ...
    WHERE api_data->>'jockey_claim_lbs' IS NOT NULL
      AND api_data->>'jockey_claim_lbs' ~ '^[0-9]+$'  -- Only valid integers
