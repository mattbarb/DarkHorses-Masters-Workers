"""
TEST: Trainers Data Capture Validation

PURPOSE:
    Validates that trainers are being correctly captured and stored into the
    ra_trainers database table via entity extraction from racecards.

WHAT IT DOES:
    1. Fetches a sample racecard from the API (UK/Ireland only)
    2. Extracts trainers from the racecard runners
    3. Inspects what fields are available from entity extraction
    4. Stores the trainers in the database via EntityExtractor
...
USAGE:
    python3 tests/test_trainers_data_capture.py
...
def main():
    """Run trainers data capture validation test"""

    print("=" * 80)
    print("TRAINERS DATA CAPTURE VALIDATION TEST")
    print("=" * 80)
    print()
    ...
    # Step 1: Fetch racecard from API to get trainers
    ...
    # Step 2: Extract trainers from runners
    print("STEP 2: Extracting trainers from runners...")
    print("-" * 80)

    # Extract trainer data manually to see structure
    trainers_extracted = {}
    for runner in runners_raw:
        ...
        if trainer_id and trainer_name:
            trainers_extracted[trainer_id] = {
                'trainer_id': trainer_id,
                'trainer_name': trainer_name,
                'trainer_location': trainer_location
            }

    if not trainers_extracted:
        print("❌ ERROR: No trainers found in runners")
        return False

    trainers_raw = list(trainers_extracted.values())
    print(f"✓ Extracted {len(trainers_raw)} unique trainers")

    if trainers_raw:
        api_fields = set(trainers_raw[0].keys())
    ...
    print("STEP 3: Storing trainers in database with **TEST** prefix...")
    print("-" * 80)

    # Manually insert test trainers with **TEST** prefix in ALL text fields
    test_trainers = []
    test_trainer_ids = []
    timestamp = datetime.utcnow().strftime('%Y%m%d%H%M%S')
    for i, trainer in enumerate(trainers_raw):
        # Add **TEST** prefix to ALL text fields
        test_id = f"**TEST**_{trainer.get('trainer_id')}_{timestamp}_{i}"
        test_trainer = {
            'id': test_id,  # **TEST** in ID
            'name': f"**TEST** {trainer.get('trainer_name')}",  # **TEST** in name
            'location': f"**TEST** {trainer.get('trainer_location')}" if trainer.get('trainer_location') else None,  # **TEST** in location
            'created_at': datetime.utcnow().isoformat(),
            'updated_at': datetime.utcnow().isoformat()
        }
        test_trainers.append(test_trainer)
        test_trainer_ids.append(test_id)

    print(f"Test trainer IDs: {test_trainer_ids}")
    print()

    # Insert directly
    db_stats = db_client.insert_trainers(test_trainers)
    result = {
    ...
    # Query the test trainers
    response = db_client.client.table('ra_trainers').select('*').in_('id', test_trainer_ids).execute()

    if not response.data:
        print("❌ ERROR: No trainers found in database")
        return False

    stored_trainers = response.data
    print(f"✓ Found {len(stored_trainers)} trainers in database")
    print()
    ...
    # Get database schema
    if stored_trainers:
        db_fields = set(stored_trainers[0].keys())
    ...
    for i, trainer in enumerate(stored_trainers, 1):
        print(f"*** TEST ENTRY #{i}: {trainer.get('name', 'Unknown')}")
        print("-" * 80)

        # Find corresponding extracted data
        # Extract original ID from test ID (format: **TEST**_{original_id}_{timestamp}_{index})
        test_id = trainer.get('id', '')
        original_id = None
        ...
        api_trainer = next((t for t in trainers_raw if t.get('trainer_id') == original_id), None)

        print(f"{'Field':<25} {'Extracted Value':<25} {'DB Value':<25}")
        print("-" * 80)

        # Show all fields
        all_fields = set(trainer.keys())
        if api_trainer:
            all_fields.update(['trainer_id', 'trainer_name', 'trainer_location'])

        for field in sorted(all_fields):
            # Map extracted field names to DB field names
            if field == 'id':
                api_field = 'trainer_id'
            elif field == 'name':
                api_field = 'trainer_name'
            elif field == 'location':
                api_field = 'trainer_location'
            else:
                api_field = field

            db_field = field

            api_val = api_trainer.get(api_field) if api_trainer else 'N/A'
            db_val = trainer.get(db_field, 'NULL')
            ...
            if db_val is None or db_val == 'NULL' or db_val == '':
                if field not in ['total_runners', 'total_wins', 'total_places', 'total_seconds',
                               'total_thirds', 'win_rate', 'place_rate', 'stats_updated_at', 'location']:
                    status = ' ⚠️  NULL'
    ...
    # Step 7: Calculate and update statistics for test trainers
    print("STEP 7: CALCULATING STATISTICS FOR TEST TRAINERS")
    print("=" * 80)
    print()
    ...
    for trainer in stored_trainers:
        ...
        if stats:
            ...
            # Update in database
            try:
                update_result = db_client.client.table('ra_trainers').update(stats).eq('id', trainer_id).execute()
                if update_result.data:
                    statistics_updated += 1
                    print(f"✓ Statistics updated for {trainer_name}")
            ...
            print(f"⚠️  No statistics calculated (trainer may have no recent results)")
        print()

    print(f"✓ Statistics calculated for {statistics_calculated}/{len(stored_trainers)} trainers")
    print(f"✓ Statistics updated in database for {statistics_updated}/{len(stored_trainers)} trainers")
    print()
    ...
    # Query the test trainers again to see updated statistics
    response = db_client.client.table('ra_trainers').select('*').in_('id', test_trainer_ids).execute()

    if response.data:
        updated_trainers = response.data
        ...
        for i, trainer in enumerate(updated_trainers, 1):
            print(f"*** TRAINER #{i}: {trainer.get('name', 'Unknown')}")
            print("-" * 80)

            for field in stats_fields_new:
                value = trainer.get(field)
    ...
    for trainer in stored_trainers:
        for field, value in trainer.items():
            if value is None and field not in old_stats_fields and field not in stats_fields_new and field != 'location':
                missing_data_count += 1

    print(f"Total test entries: {len(stored_trainers)}")
    ...
    if missing_data_count == 0 and statistics_updated > 0:
        ...
        print("✅ TEST PASSED: Trainers data capture and statistics are working correctly")
        success = True
    elif statistics_updated == 0:
        print("⚠️  No statistics were calculated (trainers may have no recent race results)")
        print("✅ TEST PASSED: Trainers data capture is working correctly")
        success = True
    else:
